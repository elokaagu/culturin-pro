...
// Blog posts data from static file
const blogPosts = [
  {
    id: 1,
...
// Function to generate tags based on category and content
function generateTags(category, title, content) {
  const baseTags = ["tour-operators", "cultural-tourism"];

  const categoryTags = {
    Marketing: ["marketing", "revenue-strategies", "business-growth"],
    "Experience Design": [
      "experience-design",
      "storytelling",
    ...
    "Industry Trends": ["industry-trends", "market-segments"],
    Technology: [
      "technology",
      "personalization",
      "customer-data",
  ...
  if (title.toLowerCase().includes("seasonal")) tags.push("seasonal-tourism");
  if (title.toLowerCase().includes("solo")) tags.push("solo-travel");
  if (title.toLowerCase().includes("barcelona"))
    tags.push("barcelona", "food-tours");
...
async function migrateBlogPosts() {
  ...
  try {
    // Transform and insert each blog post
    for (const post of blogPosts) {
      const blogPost = {
        ...
        published: true,
        published_at: convertDate(post.date),
        meta_title: `${post.title} - Culturin`,
        meta_description: post.excerpt,
        tags: generateTags(post.category, post.title, post.content),
        created_at: convertDate(post.date),
      };
    ...
    if (statsError) {
      ...
      const publishedPosts = stats.filter((post) => post.published).length;
      const categories = [...new Set(stats.map((post) => post.category))]
        .length;
