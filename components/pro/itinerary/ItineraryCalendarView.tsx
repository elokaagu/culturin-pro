...
const ItineraryCalendarView: React.FC<ItineraryCalendarViewProps> = ({
  itineraries,
  onEditItinerary,
  onQuickAction,
}) => {
  ...
  const generateCalendarDays = () => {
    ...
    for (let i = 0; i < firstDayIndex; i++) {
      days.push({ day: null, itineraries: [] });
    }
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      const dayItineraries = itineraries.filter(itinerary => {
        // Mock logic: assign itineraries to random days
        return itinerary.id.charCodeAt(0) % daysInMonth === day - 1;
      });
      days.push({ day, date, itineraries: dayItineraries });
    }
              ...
              {dayData.day && (
                <>
                  ...
                  <div className="space-y-1">
                    {dayData.itineraries.slice(0, 2).map((itinerary) => (
                      <div
                        key={itinerary.id}
                      ...
                      >
                        ...
                        <Badge
                          className={`${getStatusColor(itinerary.status)} text-white text-xs mt-1`}
                        >
                          {itinerary.status}
                        </Badge>
                      </div>
                    ...
                    {dayData.itineraries.length > 2 && (
                      <div className="text-xs text-gray-500 text-center">
                        +{dayData.itineraries.length - 2} more
                      </div>
                    )}
                  </div>
                </>
