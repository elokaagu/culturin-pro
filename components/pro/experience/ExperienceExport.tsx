...
import { ExperienceType, ExperienceModule } from "@/data/experienceData";
...
const ExperienceExport: React.FC<ExperienceExportProps> = ({
  experience,
  modules,
}) => {
  ...
  const generateShareUrl = () => {
    ...
    const url = `${window.location.origin}/experience/shared/${experience.id}`;
    setShareUrl(url);
    return url;
  };
  ...
  const exportToPDF = async () => {
    ...
    setTimeout(() => {
      ...
      element.setAttribute(
        "download",
        `${experience.title.replace(/\s+/g, "_")}_itinerary.txt`
      );
      ...
      toast({
        ...
        description: "Your experience has been exported successfully.",
      });
    }, 2000);
  };
...
CALSCALE:GREGORIAN
METHOD:PUBLISH
X-WR-CALNAME:${experience.title}
X-WR-CALDESC:${experience.description}
BEGIN:VTIMEZONE
TZID:UTC
END:VTIMEZONE
`;
...
`;
    ...
    element.setAttribute(
      "download",
      `${experience.title.replace(/\s+/g, "_")}_calendar.ics`
    );
    element.style.display = "none";
    document.body.appendChild(element);
    element.click();
  ...
  const generateTextVersion = () => {
    ...
    if (experience.price) {
      content += `Price: $${experience.price} ${experience.currency || "USD"}\n`;
    }
    if (experience.groupSize) {
      content += `Group Size: ${experience.groupSize.min}-${experience.groupSize.max} people\n`;
    }
    if (experience.difficulty) {
      content += `Difficulty: ${experience.difficulty}\n`;
    }
    content += `Duration: ${experience.days} days\n\n`;

    for (let day = 1; day <= experience.days; day++) {
  ...
  const shareViaEmail = () => {
    const subject = encodeURIComponent(`Experience: ${experience.title}`);
    const body = encodeURIComponent(
      `Check out this amazing experience I created:\n\n${generateShareUrl()}\n\n${generateTextVersion()}`
    );
      ...
      printWindow.document.write(`
        <html>
          ...
          <body>
            ...
            ${Array.from({ length: experience.days }, (_, i) => i + 1)
              .map((day) => {
                const dayModules = modules
                  .filter((m) => m.day === day)
                  .sort((a, b) => (a.position || 0) - (b.position || 0));
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <p className="text-xs text-gray-500 mt-1">
                    Anyone with this link can view the experience
                  </p>
                </div>
