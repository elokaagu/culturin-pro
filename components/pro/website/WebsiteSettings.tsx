...
const WebsiteSettings: React.FC<WebsiteSettingsProps> = ({ itineraries, setItineraries }) => {
  ...
  const saveSettings = () => {
    ...
    localStorage.setItem('culturinItineraries', JSON.stringify(filteredItineraries));
  ...
  return (
    <div className="space-y-6 max-w-3xl">
      <div>
        ...
        <p className="text-gray-600 text-sm">Manage itineraries displayed on your website</p>
      </div>
          ...
          <div className="border rounded-md overflow-hidden">
            <div className="divide-y">
              {itineraries.map((itinerary) => (
                <div key={itinerary.id} className="flex items-center space-x-3 p-3 hover:bg-gray-50">
                  <Checkbox 
                  ...
                  <div className="flex-1">
                    <label htmlFor={`itinerary-${itinerary.id}`} className="text-sm font-medium cursor-pointer">
                      {itinerary.title}
                    </label>
                    <p className="text-xs text-gray-500">
                      {itinerary.days} {itinerary.days === 1 ? 'day' : 'days'} • {itinerary.status}
                    </p>
                  </div>
                  {itinerary.image && (
                    <img 
              ...
              {itineraries.length === 0 && (
                <div className="p-6 text-center text-gray-500">
                  ...
                  <p className="text-sm mt-1">Create itineraries in the Itinerary Builder to display them on your website.</p>
                </div>
              )}
            </div>
          </div>
