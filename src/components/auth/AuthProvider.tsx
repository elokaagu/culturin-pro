...
export const AuthProvider: React.FC<AuthProviderProps> = ({ children }) => {
  ...
  const ensureUserRecord = async (user: User) => {
    try {
      ...
      if (fetchError && fetchError.code === "PGRST116") {
        ...
        console.log(
          `✅ User record exists for ${user.email} with role: ${existingUser.role}`
        );
      }
    } catch (error) {
  ...
  const createSampleItinerary = async (user: User) => {
    try {
      ...
      console.log(
        "Sample itinerary creation failed, using localStorage fallback"
      );
    }
  };
    ...
    try {
      ...
      if (typeof window !== "undefined") {
        ...
        for (let i = 0; i < localStorage.length; i++) {
          ...
          if (
            key &&
            (key.startsWith("culturinItineraries_") ||
