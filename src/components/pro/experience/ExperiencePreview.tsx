...
const ExperiencePreview: React.FC<ExperiencePreviewProps> = ({
  ...
  const [modules, setModules] = useState<ExperienceModule[]>(
    experience.modules || []
  );
  ...
  // Reset modules when experience changes
  useEffect(() => {
    if (experience.id) {
      setModules(experience.modules || []);
      setActiveDay(1);
    }
  }, [experience.id, experience.modules]);

  // Create tabs for each day in the experience
  const dayTabs = Array.from({ length: experience.days || 3 }, (_, i) => i + 1);
  ...
  const handleDrop = (e: React.DragEvent<HTMLDivElement>, day: number) => {
    ...
    try {
      ...
      const newModule: ExperienceModule = {
        ...moduleInfo,
  ...
  const handleDelete = (moduleId: string) => {
    ...
    toast({
      ...
      description: "Module has been removed from the experience",
    });
  };
  ...
  const handleModuleUpdate = (
    moduleId: string,
    updates: Partial<ExperienceModule>
  ) => {
    setModules((prev) =>
      prev.map((m) => {
        if (m.id === moduleId) {
  ...
  const handleSaveChanges = () => {
    ...
    toast({
      title: "Changes Saved",
      description: "Your experience changes have been published successfully.",
    });
  ...
  const ModuleEditForm = ({ module }: { module: ExperienceModule }) => (
    <Card className="mt-2">
      ...
      <Card>
        <CardHeader>
          ...
          <div className="flex flex-wrap gap-4 mt-4">
            ...
            {experience.price && (
              <Badge variant="outline" className="flex items-center gap-1">
                <DollarSign className="h-3 w-3" />${experience.price}{" "}
    ...
    <div className="flex flex-col h-full">
      <div className="p-4 border-b flex justify-between items-center">
        <h3 className="text-lg font-semibold">
          Travel Experience: {experience.title}
        </h3>
        <div className="flex gap-2">
                  ...
                  <div className="border-2 border-dashed rounded-lg p-4 mb-4 bg-gray-50 text-center">
                    <p className="text-gray-500">
                      {modules.filter((m) => m.day === day).length === 0
                        ? "Drag modules here to build your experience"
